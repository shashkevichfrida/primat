...
class exp():
    ...
    def rectangular_formula_right(self, h_i):
        return [sum((h_i * np.exp(x)) for x in self.array_phi)]

    def rectangular_formula_central(self, h_i):
        return [sum((h_i * np.exp(x - 0.5)) for x in self.array_phi)]

    def trapezoid_formula(self, h_i):
        return [sum((h_i / 2 * (np.exp(x - 1) + np.exp(x))) for x in self.array_phi)]

    def simpsons_formula(self, h_i):
        return [sum((h_i / 4 * (np.exp(x - 1) + 4 * np.exp(x - 0.5) + np.exp(x))) for x in self.array_phi)]
