...
class exp():
    def __init__(self):
        ...
        self.array_x = [self.a + self.h * i for i in np.arange(self.n)]
        self.x = np.linspace(self.b, self.a, int(self.n) + 1)

    # Производные
    def difference_derivative_right(self):
        return [(np.exp(x + self.h) - np.exp(x)) / self.h for x in self.array_x]
    def difference_derivative_left(self):
        return [(np.exp(x) - np.exp(x - self.h)) / self.h for x in self.array_x]

    def difference_derivative_central(self):
        return [(np.exp(x + 1) - np.exp(x - 1)) / 2 * self.h for x in self.array_x]
    ...
    def rectangular_formula(self):
        return [sum((self.h * np.sin(x - 1)) for x in self.array_x)]

    def trapezoid_formula(self):
        return [sum((self.h / 2 * (np.sin(x - 1) + np.sin(x))) for x in self.array_x)]

    def simpsons_formula(self):
        return [sum((self.h / 4 * (np.sin(x - 1) + 4 * np.sin(x - 0.5) + np.sin(x))) for x in self.array_x)]
    ...
    def func(self):
       ...
       # Площадь exp(x)
        ...
        axis1.plot(self.array_x, array_difference_derivative_right)
        ...
        axis2.plot(self.array_x, array_difference_derivative_left)
        ...
        axis3.plot(self.array_x, array_difference_derivative_central)
        ...
        axis4.plot(self.array_x, self.x * np.exp(self.x))
